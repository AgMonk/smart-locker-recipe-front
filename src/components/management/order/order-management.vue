    ...
    <el-main>
      <el-table :data="data.records">
        ...
        <el-table-column label="操作">
          <template slot-scope="s">
            <el-button type="primary" v-if="'待提交'===s.row.status" @click="submit(s.row.uuid)">提交</el-button>
            <el-button type="primary" v-if="['待提交','已提交'].includes(s.row.status)" @click="form=s.row;visible.edit=true">
              修改
            </el-button>
            ...
            <el-button type="primary" v-if="'已派单'===s.row.status" @click="complete(s.row.uuid)">提审</el-button>
            <el-button type="primary" v-if="'已派单'===s.row.status">撤单</el-button>
...
import {assignOrder, complete, confirmOrder, submit} from "../../../assets/js/api/order/order";
